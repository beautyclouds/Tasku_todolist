...
<template>
    ...
    <AppLayout :breadcrumbs="[{ title: 'History', href: '/history' }]">
        <div class="min-h-screen space-y-6 bg-[#f2f2f2] p-6 dark:bg-gray-800">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                <div
                    v-for="card in cards"
                    :key="card.id"
                    class="rounded-xl border bg-white p-5 shadow-md transition hover:-translate-y-1 hover:shadow-lg dark:bg-black"
                >
                    <!-- Card Title -->
                    <h3 class="mb-3 text-lg font-semibold">{{ card.title }}</h3>

                    <!-- Info Bar: Deadline & Closed At -->
                    <div class="mb-4 flex items-center justify-between text-sm text-gray-500 dark:text-gray-300">
                    ...
                    <div v-if="card.collaborators && card.collaborators.length > 0" class="mb-2 flex items-center gap-2">
                        ...
                        <div class="flex -space-x-2">
                            <img
                                ...
                                :alt="member.name"
                                class="h-6 w-6 rounded-full border-2 border-white object-cover shadow dark:border-gray-600"
                            />
                        </div>
                    </div>
                    ...
                    <!-- Lihat Detail -->
                    <Link :href="`/history/${card.id}`" class="inline-block text-sm font-medium text-blue-600 hover:underline"> Lihat Detail </Link>
                </div>
            </div>
        </div>
    </AppLayout>
